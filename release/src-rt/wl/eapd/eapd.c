 ...
 *
 * $Id: eapd.c 241391 2011-02-18 03:35:48Z stakita $
 */
	...
	for (i = 0; i < EAPD_WKSP_MAX_NO_BRIDGE; i++) {
		...
		foreach(name, ifnames, next) {
			if (wps_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_WPS);
			if (nas_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_NAS);
			if (i == 0 && ses_app_enabled(name)) /* SES only running on lan_ifname */
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_SES);

#ifdef BCM_DCS
			if (dcs_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_DCS);
#endif /* BCM_DCS */
		}
	}
...
static bool
eapd_add_interface(eapd_wksp_t *nwksp, char *ifname, eapd_app_mode_t mode)
{
	int unit;
	...
	if ((lanifname = get_ifname_by_wlmac(mac, ifname)) == NULL)
		return FALSE;
	...
	if (!cb) {
		...
		EAPD_INFO("add one interface cb for %s\n", cb->ifname);
	}
...
eapd_wksp_parse_cmd(int argc, char *argv[], eapd_wksp_t *nwksp)
{
	...
	while (i < argc) {
		if (!strncmp(argv[i], "-wps", 4)) {
			...
			needStart |= eapd_add_interface(nwksp, argv[i], current_mode);
		}
		i++;
	}
	return ((needStart == TRUE) ? 0 : -1);
	...
	if (status > 0) {
		...
		for (i = 0; i < nwksp->brcmSocketCount; i++) {
			...
			if (brcmSocket->inuseCount > 0 &&
				...
				/*
				 * Use read instead of recv to receive BRCM packets,
				 * this change allow the drvSocket is a file descriptor,
				 */
				bytes = read(brcmSocket->drvSocket, pkt, len);
				if (bytes > 0)  {
		...
		while (cb) {
			...
			if (preauthSocket->drvSocket >= 0 &&
				...
				/*
				 * Use read instead of recv to receive PREAUTH packets,
				 * this change allow the drvSocket is a file descriptor,
				 */
				bytes = read(preauthSocket->drvSocket, pkt, len);
				if (bytes > 0) {
#ifdef BCMDBG
